...
dx = single(header.PixelSpacing(1));    % 3D image pixel size. unit: mm
dy = single(header.PixelSpacing(2));    % 3D image pixel size. unit: mm
dz = single(header.SliceThickness);     % 3D image pixel size. unit: mm
...
nu = single(2480/4);                    % number of detector pixels along u (native: 1920 × 2480)
nv = single(1920/4);                    % number of detector pixels along v (native: 1920 × 2480)
...
%% Siddon + Projection Domain Lesion Insertion with CUDA Kernel
